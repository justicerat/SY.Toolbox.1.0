...
title KMS (Inject) Activation %ver% by mephistooo2 - TNCTR.com
cd /d "%~dp0"
...
IF "%~1"=="-ofce" GOTO :OfficeActivation
IF "%~1"=="-winds" GOTO :WindowsActivation
...
echo   Windows 7 (VL) / Windows 8 / 8.1 / 10 (ARM64) / 11 (ARM64)
echo   Windows Server 2008 / 2012 / 2012 R2 / 2016 / 2019 / 2022
echo   Office (VL) 2010 / 2013 / 2016 / 2019 / 2021
echo.
echo        NAME OS : %NameOS% %xOS%
echo        VERSION : %Version%
echo   ARCHITECTURE : %PROCESSOR_ARCHITECTURE%
echo        PC NAME : %computername%
echo ==================================================================================
echo.
echo.        [1] ACTIVATION START FOR WINDOWS ^& OFFICE
echo.
echo.        [2] ACTIVATION TASK SCHEDULER FOR WINDOWS ^& OFFICE
echo.
...
2>nul powershell -nop -c $ExecutionContext.SessionState.LanguageMode | find /i "Full" 1>nul || set _pwsh=0
...
set "_mO14c=Detected Office 2010 C2R Retail is not supported by KMS Inject"
set "_mO14m=Detected Office 2010 MSI Retail is not supported by KMS Inject"
set "_mO15m=Detected Office 2013 MSI Retail is not supported by KMS Inject"
set "_mO16m=Detected Office 2016 MSI Retail is not supported by KMS Inject"
set "_mOuwp=Detected Office 365/2016 UWP is not supported by KMS Inject"
...
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*EvalEdition~*.mum" set "nEval=De§erlendirme Sunucu iŸletim sistemleri etkinleŸtirilemez. Ltfen tam Sunucu isletim iŸletim ykleyin."
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*EvalCorEdition~*.mum" set _eval=1&set "nEval=De§erlendirme Sunucu iŸletim sistemleri etkinleŸtirilemez. Ltfen tam Sunucu iŸletim sistemi ykleyin."
set "_C16R="
...
if %ActWindows% EQU 0 (
  ...
  if %SSppHook% EQU 0 (
    ...
    echo.&echo Failed checking KMS Activation ID^(s^) for Windows.&echo Either sppsvc service or SppExtComObjHook.dll is not functional.&call :CheckWS
    exit /b
    )
  )
)
...
if %winbuild% GEQ 9200 (
  if %OffUWP% EQU 0 (echo.&echo.) else (echo.&echo %_mOuwp%)
  exit /b
  )
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo.&exit /b))
)
...
if %WinPerm% EQU 1 (
for /f "tokens=2 delims==" %%x in ('%_qr%') do echo.
echo. %NameOS% %xOS% is Permanently Activated.
exit /b
)
call :insKey
exit /b
...
if %OsppHook% EQU 0 (echo.&echo.&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo.&exit /b)
set err_offsvc=0
...
if "%ls3%"=="1" (
echo. %~6 is Permanently Activated.
exit /b
)
if "%ls2%"=="1" (
echo.
echo. %~4 is Permanently Activated.
exit /b
)
if "%ls%"=="1" (
echo.
echo. %~2 is Permanently Activated.
exit /b
)
call :insKey
exit /b
...
%echoRed% Product Activation Failed: 0xC004F035
...
%echoRed% Product Activation Failed
)
echo Remaining Period: %gpr2% days ^(%gpr% minutes^)
set S_OK=0
exit /b
...
::===============================================================================================================
:InstallHook
for %%# in (KMS.dll,SppExtComObjHookAvrf.dll,SppExtComObjHook.dll,SppExtComObjPatcher.dll,SppExtComObjPatcher.exe) do (
...
echo "%SystemRoot%\System32\KMS.dll"
echo.
...
set rancopp=0
if %_Retail% EQU 0 if %_OMSI% EQU 0 if %_pwsh% equ 1 (
set rancopp=1
pushd "!_work!\bin"
...
if %_C16Msg% EQU 1 (
echo.
%echoGreen% Converting Office C2R Retail-to-Volume:
)
...
copy /y KMSInject.bat %windir%\KMS\ >nul 2>&1
...
:TaskDelete
echo.
schtasks /delete /tn "KMS_Aktivasyon" /f >nul 2>&1
...
copy /y "%0" "%systemdrive%\$OEM$\$$\Setup\Scripts"\run.bat >nul 2>&1
...
:Exit
pushd %temp%
del /f /q KMS_Suite.bat >nul 2>&1
echo MSGBOX "SPECIAL THANKS : TNCTR Family - CODYQX4, abbodi1406, qewlpal, cynecx, qad, Mouri_Naruto (MDL), WindowsAddict, mspaintmsi, BAU (AveYo)", vbInformation,"..:: mephistooo2 | TNCTR ::.."  > %temp%\TEMPmessage.vbs
TEMPmessage.vbs
del /f /q TEMPmessage.vbs >nul 2>&1
mode con cols=55 lines=4
echo.
echo Files are being deleted from the temp directory...
echo.
for /r %temp% %%a in (*) do if "%%~nxa"=="disableX.ps1" set p=%%~dpa
rd /s /q %p%  >nul 2>&1
del /f /q %0% >nul 2>&1
::===============================================================================================================
:Color_Pre
...
::===============================================================================================================
:Check
title Windows ^& Office Activation Status Check
@set "0=%~f0" &powershell -nop -c $f=[IO.File]::ReadAllText($env:0)-split':check\:.*';iex($f[1]); X(1) >nul
call check.vbs
GOTO MAINMENU

:check:
$k='X0Isr@=S9_fP{|H?z$gT[c>MNo(%eAh^mvq*;1wy6b-,C}LkE7~xuO\aWdZGlQ/8!)`pUnjRJ+#&5Bt<i4]YD32.FKV'; Add-Type -Ty @'
...
:check:[ check_.vbs
::%\D~$XXX0}t0XXXXXX2hXXXXXXXX8@zX!XXXjtT0XXSsXX10nv*S<}XXXXXXXXkp#G4p!@c7Bl2qx6;a|4jt|&^xTWy%{5oP}_@V<T0XNWEpe*IVtMJpQo0Hp.8Ef)Fdj*
::]IM-O6(B]cmQv&U>+chGz4[9`bCLOYJc1K0X$X(gxX[r40\D_uz=~Mcoyve-KM.P-(-[rz{8G^Ns/2U\.x#qq,,!`UAnBkv?s6Ru.+2vNe`9{(?_CJ*@wD0V0V>.^XrXkp
::[Xs\]@E!s[_ao5?ccS+@%M]gX*FPddZ55bs.!P6x>/9H=[/}Y%I}&+X;%#wLP(j{{w9Pt-lu$aH7~5m)yu?Sr^Uo&/%8/\Nil{!G75D%0/`KO]{!Kk=sD&*^4v<720KAN3
